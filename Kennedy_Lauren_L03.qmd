...
```{r}
#| label: ex1plot1
plot_base <-ggplot(cdc,aes(height,weight))+
  ylab("Weight (lbs)")+
  xlab("Height (in)")+
  theme_minimal()
...
ggplot(cdc,aes(height,weight))+
  ...
  geom_smooth(aes(color = gender), method = "lm", se=F, fullrange = TRUE)+
  theme_minimal() +
  ylab("Weight (lbs)")+
  xlab("Height (in)")
```
...
#| 
plot_base+
  geom_density_2d(aes(color=gender),lwd=0.7)
```
...
- use position_dodge to jitter points
- try geom_errorbarh
...
```{r}
#| label: ex-3plot
ggplot(cdc_weight_95ci,aes(x=mean_wt, y=gender,
                     color=genhlth))+
